 ...
 */
public class ConfexDiagramEditor extends GraphicalEditorWithFlyoutPalette
	...
	public void init(IEditorSite site, IEditorInput input)
		...
		// System.err.println("[SchemaDiagramEditor.init]
		// Display.getCurrent()==null Не могу создать new SchemaClipboard");
		// clipboard = null;
		// } else {
		// }
	}
	...
	public void doSave(IProgressMonitor monitor) {
		try {
			 ...
			 * objectOut.writeObject(schema); objectOut.close();
			 */
			IFile file = ((IFileEditorInput) getEditorInput()).getFile();
			schema.saveAsXml(file, monitor);
			/*
			 * file.setContents(new ByteArrayInputStream(out.toByteArray()),
			 * true, false, monitor); out.close();
			 */
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	protected void createGraphicalViewer(Composite parent) {
		...
		// ContextMenuProvider provider = new SchemaContextMenuProvider(viewer,
		// getActionRegistry());
		ContextMenuProvider provider = new DefaultContextMenuProvider(viewer,
				getActionRegistry());

		viewer.setContextMenu(provider);
		getSite().registerContextMenu("net.confex.schema.editor.contextmenu",
				provider, viewer);
	...
	protected KeyHandler getCommonKeyHandler() {
		...
		sharedKeyHandler.put(KeyStroke.getPressed(SWT.CTRL | 'C', 0),
				getActionRegistry().getAction(GEFActionConstants.COPY));
		sharedKeyHandler.put(KeyStroke.getPressed(SWT.CTRL | 'P', 0),
				getActionRegistry().getAction(GEFActionConstants.PASTE));
	...
	protected void createActions() {
		...
		// addEditPartAction(new DeleteAction((IWorkbenchPart) this));
		// addEditorAction(new CopyActionOld(this));
		addEditPartAction(new CopyAction(this));
		// addEditPartAction(new PasteSelectionAction(this));
		addAction(new MenuActions.PasteAction(this));
		// addEditorAction(new CopyAction(this));

		// -- добавляем свое действие в реестр дейстрий редактора
		addAction(new MenuActions.AddLabelAction(this));
		addAction(new MenuActions.AddContainerAction(this));

		// addAction(new MenuActions.PasteActions(this));
	...
	private static ImageDescriptor create(String iconPath, String name) {
		return AbstractUIPlugin.imageDescriptorFromPlugin(
				"net.confex.schema.schemaeditor", iconPath + name);
	}
	...
	protected void configureGraphicalViewer() {
		...
		viewer.setContextMenu(provider);
		getSite().registerContextMenu("net.confex.schema.edito.DefaultContextMenuProvider", //$NON-NLS-1$
				provider, viewer);
	...
	protected void loadProperties() {
		...
		*/
		...
		if (manager != null)
			manager.setZoom(getSchema().getZoom());
		// Scroll-wheel Zoom
